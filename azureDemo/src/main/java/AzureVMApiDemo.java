...
public class AzureVMApiDemo {
     ...
     * args7: instance type
     * args8: instance number
     */
    public static String main(String[] args) throws Exception {

        if (args[7].contains("dc")) {
            size = "Standard_D1";
            vmName = String.format("dc%s%s", args[8], "vm");
            resourceGroupName = String.format("dc%s%s", args[8], "rg");
        } else if (args[7].contains("lg")) {
            size = "Standard_D1";
            vmName = String.format("lg%s%s", args[8], "vm");
            resourceGroupName = String.format("lg%s%s", args[8], "rg");
        } else {
            size = VirtualMachineSizeTypes.STANDARD_A0;
            vmName = String.format("fe%s%s", args[8], "vm");
            resourceGroupName = String.format("fe%s%s", args[8], "rg");
        }
        ...
        String dns = vmName + ".eastus.cloudapp.azure.com";
        return dns;
    }
}
